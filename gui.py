...
def run_gui():
    ...
    def save_drawing(filename=None):
        if filename is None or not isinstance(filename, str):
            filename = f"spirograph_{timestamp()}.png"
    ...
    try:
        ...
        surface = pygame.display.set_mode(pygame.display.list_modes()[0])
        draw_surface = pygame.Surface(pygame.display.list_modes()[0])
    ...
    buttons = [
        ...
        UIButton('BG Color', DEFAULT_WIDTH-110, DEFAULT_HEIGHT-40, 100, 30, change_bg_color, 16, WHITE, BLUE, GREY)
    ]
    ...
    while running:
        ...
        # blit draw_surface onto main surface
        ...
        # draw buttons and possibly gear config ui
        for button in buttons:
            button.draw(surface)
        if show_gear_config:
            widget_rect = pygame.Rect(250, 120, 300, 180)
